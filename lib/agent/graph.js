...
/**
 * 1. RESEARCHER NODE
 * Gathers news and data. Uses Serper Search API if key is present,
...
async function researcherNode(state) {
  ...
  try {
    if (process.env.SERPER_API_KEY) {
      logs.push(`[Researcher] Querying Serper Search API for "${company}" financial news...`);
      ...
      logs.push(`[Researcher] Serper search complete.`);
      return {
      ...
      logs.push(`[Researcher] No SERPER_API_KEY found. Fetching LLM historical knowledge report for "${company}"...`);
